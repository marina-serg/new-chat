...
class JoinAndLeave(AsyncWebsocketConsumer):
    ...
    async def leave_group(self, group_uuid):
        group = await database_sync_to_async(Group.objects.get)(uuid=group_uuid)

        await database_sync_to_async(group.remove_user_from_group)(self.user)

        members_count = await database_sync_to_async(group.members.count)()
        data = {
            "type": "leave_group",
            "data": {
        ...
        # Отправляем данные клиенту перед удалением группы
