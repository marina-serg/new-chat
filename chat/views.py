...
def home_view(request):
    groups = Group.objects.all().annotate(last_message=Subquery(Message.objects.filter(group=OuterRef('pk')).order_by('-timestamp').values('content')[:1]))
    group_info = []
    for group in groups:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            group.save()
            group.add_user_to_group(request.user)  # Добавляем создателя в группу автоматически
            return redirect(reverse('group',args=[group.uuid])) # Перенаправляем на чат группы
    else:
        form = GroupForm()
